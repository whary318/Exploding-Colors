...
// Call this method whenever you want to update the boardView on the display from the current status of the board
	...
	public int processTouchedTile(TileView tv, int min) {
		// TODO
		...
		int score = 0;
		b.locateNeighbors(tv.getRow(), tv.getCol(), tv.getColor(), set);
		if(set.size() >= min) {
            b.removeMatchingTiles(set);
            b.collapseColumns();
            score = (min * 10) + ((set.size() - min) * 25);
        }
		...
		return score; // you figure out the score
	}
